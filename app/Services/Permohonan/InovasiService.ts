...
class InovasiService {
  public async lists(opd_uuid: string) {
    const model = await Inovasi.query()
      .preload("opd")
      .where("opd_uuid", opd_uuid)
      .orderBy("id", "asc");
  ...
  public async store(payload: InovasiType, user: UserType) {
    try {
      ...
      //cari category UUID
      const category: any = await Category.findBy("code", "IID");

      //create inovasi indikator
      const indikators = await Indikator.query()
  ...
  public async verifikatorshow(id: string) {
    const model = await Inovasi.query()
      .preload("opd")
      .preload("category")
      .preload("jenisinovasi")
      .preload("urusan")
      .preload("bentuk")
      .preload("regency")
      .preload("district")
      .preload("village")
      .preload("inovasiindikators", (trxQuery) => {
        trxQuery
          .preload("indikator")
          .preload("inovasidocuments")
          .preload("inovasiinformasi");
      })
      .where("uuid", id)
      .first();

    let data = model?.dataverifikatorrecord;
    data["path_file_anggaran"] =
      Env.get("BASE_URL") +
      (await Drive.getSignedUrl("documents/" + model?.fileAnggaran));
    data["path_file_profil_bisnis"] =
      Env.get("BASE_URL") +
      (await Drive.getSignedUrl("documents/" + model?.fileProfilBisnis));
    data["path_foto_inovasi"] =
      Env.get("BASE_URL") +
