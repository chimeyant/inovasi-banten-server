...
class InovationIndicatorService {
  ...
  public async lists(inovation_uuid:string){
    const model = await this.Model.query().where('inovation_uuid', inovation_uuid).orderBy("id",'asc')
  ...
  public async update(payload:InovationIndicatorType, id:string, inovation_uuid: string){
    try {
      const model = await this.Model.findBy("uuid",id )
      model?.merge({
        score:payload.score
      ...
      const jmlscore = await this.Model.query().knexQuery.sum('score').whereRaw('inovation_uuid = ?',[inovation_uuid] )

      await Inovation.query().where("uuid", payload.inovation_uuid).update({finnaly_score: jmlscore[0].sum})
