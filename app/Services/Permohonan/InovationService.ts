import {
  ...
  MSG_FAILED_PROCESS,
  MSG_PUBLISH_SUCCESS,
  MSG_SEND_PERMOHONAN_SUCCESS,
  MSG_STORE_SUCCESS,
  MSG_UNPUBLISH_SUCCESS,
  MSG_UNSEND_PERMOHONAN_SUCCESS,
  MSG_UPDATE_SUCCESS,
...
class InovationService {
  ...
  public async lists(payload: any) {
    ...
    if (payload.user.authent == "kompetisi") {
      const model = await this.Model.query()
        .preload("kompetisi")
        .where("category_uuid", payload.category.id)
        .where("user_uuid", payload.user.id)
        .orderBy("created_at", "desc");
  ...
  public async store(payload: SinovicType) {
    try {
      ...
      model.tipe = payload.tipe;
      model.suratPernyataanKetersediaanReplikasi =
        payload.surat_pernyataan_ketersediaan_replikasi;
  ...
  public async show(id: string) {
    ...
    const data = {};
    const property = {
      path_surat_pernyataan_implementasi:
        Env.get("BASE_URL") +
        (await Drive.getSignedUrl(
          "documents/" + model?.suratPernyataanImplementasi
        )),
      path_surat_pernyataan_identitas:
        Env.get("BASE_URL") +
        (await Drive.getSignedUrl(
          "documents/" + model?.suratPernyataanIdentitas
        )),
      path_surat_pernyataan_ketersediaan_replikasi:
        Env.get("BASE_URL") +
        (await Drive.getSignedUrl(
          "documents/" + model?.suratPernyataanKetersediaanReplikasi
        )),
      path_ringkasan_att:
        Env.get("BASE_URL") +
        (await Drive.getSignedUrl("documents/" + model?.ringkasanAtt)),
      path_latar_belakang_att:
        Env.get("BASE_URL") +
        (await Drive.getSignedUrl("documents/" + model?.latarBelakangAtt)),
      path_kebaruan_att:
        Env.get("BASE_URL") +
        (await Drive.getSignedUrl("documents/" + model?.kebaruanAtt)),
      path_implementasi_att:
        Env.get("BASE_URL") +
        (await Drive.getSignedUrl("documents/" + model?.implementasiAtt)),
      path_signifikansi_att:
        Env.get("BASE_URL") +
        (await Drive.getSignedUrl("documents/" + model?.signifikansiAtt)),
      path_adaptabilitas_att:
        Env.get("BASE_URL") +
        (await Drive.getSignedUrl("documents/" + model?.adaptabilitasAtt)),
      path_sumber_daya_att:
        Env.get("BASE_URL") +
        (await Drive.getSignedUrl("documents/" + model?.sumberDayaAtt)),
      path_strategi_keberlanjutan_att:
        Env.get("BASE_URL") +
        (await Drive.getSignedUrl(
          "documents/" + model?.strategiKeberlanjutanAtt
        )),
      path_foto:
        Env.get("BASE_URL") +
        (await Drive.getSignedUrl("documents/" + model?.foto)),
    };
  ...
  public async update(payload: SinovicType, id: string) {
    try {
      ...
      model?.merge({
        ...
        tipe: payload.tipe,
        suratPernyataanKetersediaanReplikasi:
          payload.surat_pernyataan_ketersediaan_replikasi,
  ...
  public async send(id: string, user: any) {
    try {
      ...
      const register =
        "REG." +
        DateTime.fromJSDate(currdate).toFormat("yyyyMM") +
        "." +
        string.generateRandom(4);
      ...
      const pesan =
        ...
        "\r\n\r\nTerima Kasih, permohonan Inovasi Anda Dengann Nomor Registrasi " +
        register +
    ...
    try {
      ...
      if (status == "2") {
        const pesan =
          ...
          "\r\n\r\nMohon maaf permohonan anda dengan nomor registrasi : " +
          model?.noreg +
          " ditolak, silahkan perbaiki terlebih dahulu \r\n\r\nSalam Inovasi,\r\nProvinsi Banten";
      ...
      if (status == "4") {
        const pesan =
          "*INOVASI BANTEN* \r\n `Jaringan Inovasi Banten` \r\n\r\nHalo... \r\n" +
          model?.inovatorNama.toUpperCase() +
          "\r\n\r\nSelamat permohonan anda dengan nomor registrasi : " +
          model?.noreg +
          " telah berhasil terverifikasi pada tahapan verifikasi dokumen dan akan memasuki proses tahapan verifikasi substansi \r\n\r\nSalam Inovasi,\r\nProvinsi Banten";
      ...
      if (status == "5") {
        const pesan =
          ...
          "\r\n\r\nSelamat permohonan anda dengan nomor registrasi : " +
          model?.noreg +
          " telah berhasil terverifikasi pada tahapan verifikasi Substansi dan akan memasuki proses tahapan publikasi \r\n\r\nSalam Inovasi,\r\nProvinsi Banten";
  ...
  public async toplists() {
    ...
    model.forEach(async (element) => {
      ...
      const foto =
        Env.get("BASE_URL") +
        (await Drive.getSignedUrl("documents/" + element?.foto));
      Object.assign(
        row,
        element.datadisplay,
        { kompetisi: element.kompetisi.name },
  ...
  public async dashboardPublic(user: any) {
    try {
      ...
      const jmlpermohonan = await this.Model.query()
        .where("user_uuid", user.id)
        .getCount();

      const jmldraftpermohonan = await this.Model.query()
        .where("user_uuid", user.id)
        .whereIn("status", ["0", "1", "3"])
        .getCount();

      const jmlpermohonanproses = await this.Model.query()
        .where("user_uuid", user.id)
        .whereIn("status", ["4"])
        .getCount();

      const jmlpermohonanpublish = await this.Model.query()
        .where("user_uuid", user.id)
        .whereIn("status", ["5"])
        .getCount();
  ...
  public async datachartperbulan() {
    try {
      ...
      const januari = await this.Model.query()
        .whereIn("status", ["1", "2", "3", "4", "5"])
        .whereRaw("created_at::text like ?", ["2023-01%"])
        .getCount();
      const pebruari = await this.Model.query()
        .whereIn("status", ["1", "2", "3", "4", "5"])
        .whereRaw("created_at::text like ?", ["2023-02%"])
        .getCount();
      const maret = await this.Model.query()
        .whereIn("status", ["1", "2", "3", "4", "5"])
        .whereRaw("created_at::text like ?", ["2023-03%"])
        .getCount();
      const april = await this.Model.query()
        .whereIn("status", ["1", "2", "3", "4", "5"])
        .whereRaw("created_at::text like ?", ["2023-04%"])
        .getCount();
      const mei = await this.Model.query()
        .whereIn("status", ["1", "2", "3", "4", "5"])
        .whereRaw("created_at::text like ?", ["2023-05%"])
        .getCount();
      const juni = await this.Model.query()
        .whereIn("status", ["1", "2", "3", "4", "5"])
        .whereRaw("created_at::text like ?", ["2023-06%"])
        .getCount();
      const juli = await this.Model.query()
        .whereIn("status", ["1", "2", "3", "4", "5"])
        .whereRaw("created_at::text like ?", ["2023-07%"])
        .getCount();
      const agustus = await this.Model.query()
        .whereIn("status", ["1", "2", "3", "4", "5"])
        .whereRaw("created_at::text like ?", ["2023-08%"])
        .getCount();
      const september = await this.Model.query()
        .whereIn("status", ["1", "2", "3", "4", "5"])
        .whereRaw("created_at::text like ?", ["2023-09%"])
        .getCount();
      const oktober = await this.Model.query()
        .whereIn("status", ["1", "2", "3", "4", "5"])
        .whereRaw("created_at::text like ?", ["2023-10%"])
        .getCount();
      const nopember = await this.Model.query()
        .whereIn("status", ["1", "2", "3", "4", "5"])
        .whereRaw("created_at::text like ?", ["2023-11%"])
        .getCount();
      const desember = await this.Model.query()
        .whereIn("status", ["1", "2", "3", "4", "5"])
        .whereRaw("created_at::text like ?", ["2023-12%"])
        .getCount();
  ...
  public async datachartperproses() {
    ...
    const pengajuan = await this.Model.query()
      .whereIn("status", [1, 3])
      .getCount();
