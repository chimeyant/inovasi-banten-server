...
export default class InovasiDocumentsController {
  ...
  public async store({ params, request, response }: HttpContextContract) {
    ...
    const { nomor_dokumen, tanggal_dokumen, tentang, type_file, file_dokumen } =
      request.all();
  ...
  public async show({ params }: HttpContextContract) {
    const result = await InovasiDocumentService.show(params.id);
  ...
  public async destroy({ params, response }: HttpContextContract) {
    const result = await InovasiDocumentService.delete(params.id);
